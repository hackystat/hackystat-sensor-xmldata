...
/**
 * The option that sets the verbose mode to be on or off. When verbose mode is on, additional
...
public class VerboseOption implements Option {
  ...
  private String[] verboseValuesArray = { "true", "false", "yes", "no", "on", "off" };

  /**
   * Returns true if the parameters contains only one value and the value is either 'true', 'yes',
  ...
  public void set (XmlDataOutputParser parser, Object parameters) throws XmlSensorException {
    ...
    if (this.validate()) {
      XmlDataController.setVerbose(this.verboseToBoolean((String) this.parameters.get(0)));
      parser.changed("Verbose mode on: " + XmlDataController.isVerbose());
    }
    else {
      XmlDataController.notifyOptionListeners(new OptionEvent("Failed to set the "
          + "verbose mode.  To enable verbose mode use 'true', 'on', or 'yes'.", false));
    }
  }

  /**
   * Returns true if the String accepted is 'true', 'on', or 'yes'. Returns false if any other value
   * is accepted. This method was written as a replacement for the Ant method Project.toBoolean().
   * The Command-Line Interface would require the ant.jar library in order to support the
   * Project.toBoolean() method, so I created a little hack to mimic its features.
   * 
  ...
  private boolean verboseToBoolean (String verboseString) {
    if (verboseString.equalsIgnoreCase("true") || verboseString.equalsIgnoreCase("on")
